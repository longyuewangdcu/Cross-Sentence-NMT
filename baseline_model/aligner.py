...
class Aligner(object):
    ...
    def apply(self, output, verbose=False):
        ...
        while 1:
            ...
            alignment = numpy.array(results[0]).transpose()
            idx = 1
            ...
            if verbose:
                ...
                # added by Zhaopeng Tu, 2016-05-04
                if self.with_coverage:
                    logger.info("Coverage: {}".format(self._idx_to_word(source, self.idict_src, coverage)))
                    if self.coverage_type is 'linguistic':
                        logger.info("Fertility: {}".format(self._idx_to_word(source, self.idict_src, fertility)))
